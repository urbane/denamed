...
function expandZone(zone: Zone) {
    ...
    (['a', 'txt', 'cname', 'mx', 'ns', 'aaaa'] as (keyof Zone)[]).forEach(key => {
        if (zone[key]) {
            newZone[key] = {} as any;
            Object.keys(zone[key] as any).forEach(name => {
                const value = (zone[key] as any)[name]
                newZone[key][(name.endsWith(zone.tld) ? name : name + '.' + zone.tld) as string] = typeof value === 'string' ? {value} : value;
            })
        }
    })
    return newZone
...
function matchAnswerInZone(expandedZone: Zone, question: DnsQuestion): DnsAnswer | undefined {
    if (question.name.endsWith(expandedZone.tld)) {
        switch (question!.type) {
            case "A": {
                const match = expandedZone.a?.[question.name] as ValueWithTtl;
                if (match)
                    return createAAnswer(question, match.value, match.ttl ?? expandedZone.ttl)
            }
                break;
            case "NS": {
                const match = expandedZone.ns?.[question.name] as ValueWithTtl;
                if (match)
                    return createNSAnswer(question, match.value, match.ttl ?? expandedZone.ttl)
            }
                break;
            case "MX": {
                const match = expandedZone.mx?.[question.name] as MxValue;
                if (match)
                    return createMxAnswer(question, match.value, match.priority, match.ttl ?? expandedZone.ttl)
            }
                break;
            case "TXT": {
                const match = expandedZone.txt?.[question.name] as ValueWithTtl;
                if (match)
                    return createTxtAnswer(question, match.value, match.ttl ?? expandedZone.ttl)
            }
                break;
            case "CNAME": {
                const match = expandedZone.cname?.[question.name] as ValueWithTtl;
                if (match)
                    return createCnameAnswer(question, match.value, match.ttl ?? expandedZone.ttl)
            }
                break;
            case "AAAA": {
                const match = expandedZone.aaaa?.[question.name] as ValueWithTtl;
                if (match)
                    return createAaaaAnswer(question, match.value, match.ttl ?? expandedZone.ttl)
            }
                break;
        }
